...
function getWorksheet(filename) {
    try {
        log(`Reading in worksheet ${filename}`);
...
function getAddresses(worksheet) {
    try {
        ...
        while(!end_of_column) {
            ...
            // guard against cells not existing, if undefined pus null into array
            if(worksheet[cell] || worksheet[nextCell]) {
                addresses.push(worksheet[cell] ? worksheet[cell].h : null);
                row++;
            } else {
                end_of_column = true;
            }
